...
export default {
  ...
  mounted() {
    this.$store.dispatch("getDetailList", this.$route.params.goodsId);
  },
  computed: {
    ...mapGetters(["categoryView", "skuInfo", "spuSaleAttrList"]),
    // 当服务器还未返回数据时，就当做空数组而不是undefined
    skuImageList() {
  ...
  methods: {
    ...
    // 添加购物车，查看是否成功。这里只需要返回状态，不需要返回数据，因此不需要三连环
    addShopCar() {
      // 派发请求到服务器，用于保存加入购物车的商品
      // 服务器保存成功，进行路由跳转传递参数
      // 服务器保存失败，给用户进行提示
      let params = {
      ...
      try {
        this.$store.dispatch("getAddShopCartState", params);
        // 先产品信息保持到会话存储中，将路由跳转
